...
class KonataRenderer{
     ...
     * @param {Op} op 
     */
    getStageColor_(laneName, stageName, isBegin, op,alpha){
        let self = this;
        ...
        if (self.colorScheme_ == "Auto" || self.colorScheme_ == "Unique") {
            ...
            if (isBegin) {
                ...
                return `hsla(${h},${color.sBegin}%,${color.lBegin}%,${alpha})`;
            }
            else{
                let h = ((250 - level*color.hRateEnd + laneID*28*8)%360);
                return `hsla(${h},${color.sEnd}%,${color.lEnd}%,${alpha})`;
            }
        }
    ...
    */
    ...
    getPipelineToolTipText(x, y){
        ...
        // X 座標に対応したサイクル数を取得
        let base  = self.getCycleFromPixelPosX(0);
        let cycle = self.getCycleFromPixelPosX(x);
        let text = `[Cycle:${cycle-1}, RelativeCycle:${cycle-base} ,ID:${op.id} ,RID:${op.rid}]\n[${op.labelName}]\n`;
    ...
    */
    ...
    drawLane_(op, h, startCycle, endCycle, scale, ctx, laneName){
        ...
        //     if(store.activeTab.selThread.indexOf(op.tid)!=-1)
        //         this.alpha = 1;
        
        // if(["all","stage"].indexOf(store.activeTab.visMode)!=-1){
        //     this.alpha = 1;
        // }
        ...
        for (let i = 0, len = lane.length; i < len; i++) {
            ...
            if(store.activeTab.selThread.length==0){
                if(store.activeTab.selStage.length==0){
                    this.alpha = 1;
                }else{
                    if(store.activeTab.selStage.includes(stage.name))
                        this.alpha = 1;
                    else
                        this.alpha = 0.2;
                }
                ...
                if(store.activeTab.selStage.length==0){
                    if(store.activeTab.selThread.includes(op.tid))
                        this.alpha = 1;
                    else
                        this.alpha = 0.2;
                }else{
            ...
            grad.addColorStop(0, self.getStageColor_(laneName, stage.name, true, op,this.alpha));
            grad.addColorStop(1, self.getStageColor_(laneName, stage.name, false, op,this.alpha));
